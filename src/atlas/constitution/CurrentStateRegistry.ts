...
const CAPABILITY_STATE: Record<string, CapabilityStateDefinition> = {
  planning: {
    ...
    evidence: [
      "Context/Planner integration built (2026-07-11): closed the exact gap the old 'Gap to 1.0' bullet below named — PlannerEngine/PlannerRegistry was real but only ever wired into the standalone Proof-of-Power demo, never the real mission loop. Five new ExecutionPlanStepKinds were added for engineering/content/tips missions (context-gather, implement, approval-gate, apply, validate — see planner.types.ts), two new deterministic planners were built (executionPlanner, contentMissionPlanner, tipsPlanner in planners/defaultPlanners.ts, replacing a reliance on the demo's fuzzy matchScore text-matching), and ExecutionPlan gained an optional missionId so a plan can be found and updated again later by mission id, independent of whatever else has since become 'current'",
      "atlas-runtime.ts's ensureExecutionProposal() now registers a real plan (buildPlanForMission()) for every mission BEFORE dispatching to the content/tips/execution engine — deterministic, using the exact same isContentMission/isTipsMission branch the dispatch itself uses, not a second, possibly-disagreeing classification",
      "The four real engines (contentGenerationEngine.ts, tipsGenerationEngine.ts, executionEngine.ts, applyEngine.ts) call updatePlanStepByKind()/updatePlanStatusById() at their real checkpoints (running/completed/failed/skipped) — best-effort by construction (wrapped, never throws), so a planner hiccup can never block real mission work. applyEngine.ts also resolves the approval-gate step the moment it runs (this function only ever executes after an explicit CEO Approve click) and marks the whole plan status 'completed' once code lands in the working tree, regardless of post-apply validation outcome — a failed validation drafts its own separate FIX-XXX mission with its own separate plan, it doesn't reopen the original one",
      "The live plan is now serialized into public/atlas-runtime-state.json every cycle (best-effort, never blocks the snapshot write if the mission's plan aged out of the capped executionQueue) and rendered in Atlas Control as a new 'Nu bezig' section (LivePlanSectionV2.tsx) showing every step's real status — this is the actual dashboard-visibility half of the gap, not just backend plumbing",
      ...
      "Default planners registered (recipe/publishing/knowledge content workflows, plus the three new mission-shaped planners above)",
      ...
      "Superseded — kept for history only: the bullet below ('Gap to 1.0: PlannerEngine/PlannerRegistry ... only wired into one demo workflow') was true when written but is no longer accurate as of the Context/Planner integration above. Genuine remaining gap to 1.0: step counts per mission kind are still fixed/templated (5 steps for execution, 3 for content/tips), not dynamically generated from the mission's actual real-world complexity, and there is still no broader re-planning if a mission's real-world outcome diverges from its plan in ways post-apply validation can't detect (e.g. a CEO rejects a proposal for reasons validation can't catch)",
      "BRAIN-011 built: closed the exact gap this entry used to name — 'no iterative re-planning if an execution attempt fails validation'. scripts/atlas/replanOnFailure.ts now runs right after EXEC-002/EXEC-003's post-apply validation: if typecheck or the test suite failed, it writes a real FIX-XXX mission file (with the actual failure output as its Goal) and drafts a real code proposal for it via the same Execution Engine every other mission uses — never applied automatically, always a fresh CEO Inbox item awaiting an explicit Approve",
      "Real architectural gap found and fixed while wiring this: the CEO Inbox's 'Open' tab (buildCandidateApprovals in realCompanyData.ts) is built around a single activePackage — whatever the Decision Engine currently ranks top, or whatever's in the one-slot CEO-instruction file. A freshly drafted fix proposal would NOT automatically win that slot and would silently sit invisible. Deliberately did not force it into the CEO-instruction slot either, since that could silently override an explicit CEO request (e.g. a queued content retry) — instead gave fix proposals their own always-surfaced lane (pendingFixMissions), visible regardless of whatever else is currently active",
      "Idempotent by construction: a fix-mission.json marker in the applied-<timestamp> archive folder prevents the same validation failure from drafting a duplicate fix proposal twice",
      "Bounded/safe even in the worst case: if a FIX-XXX mission's own apply somehow also fails validation, it just drafts another FIX-XXX behind it — every step still requires a human Approve click, so there's no autonomous loop that can run away or touch the working tree without review",
  ...
  context: {
    ...
    evidence: [
      ...
      "Superseded (2026-07-11): the bullet below was accurate when written — plannerEngine.plan() really was only ever called from the standalone Proof-of-Power demo, disconnected from the real mission pipeline. That is now fixed: atlas-runtime.ts's ensureExecutionProposal() calls buildPlanForMission()/registerPlan() for every real mission before dispatching to its engine (see the 'planning' capability entry above for the full breakdown). The specific open design question the old bullet raised — 'should PlannerEngine's model even apply to self-review cycles' — was answered empirically rather than left open: yes, once given step kinds actually shaped for engineering work (context-gather/implement/approval-gate/apply/validate) instead of only the content-pipeline kinds that existed before",
      "Gap to 1.0, corrected after actually investigating (not assumed) — kept for history, superseded by the bullet above: this entry used to say 'few callers pass plannerOutput in yet', implying an easy wire-up. Traced every real call site: AutonomousDecision.ts's gatherContext() (the main self-review caller) never has an ExecutionPlan in scope at all; the only place plannerEngine.plan() is ever actually called anywhere in the repo is the standalone Proof-of-Power demo workflow, fully disconnected from the real mission pipeline (orchestrateMission() never imports anything from brain/planner). deriveContextHealth() requiring providers.length > 0 for 'healthy' means snapshots taken when no AI provider is configured will always read 'partial', which is arguably correct signal, not a bug — not changed here. Genuine remaining gap: AutonomousDecision.ts's gatherContext() still never reads the live plan back into its own context snapshot — the plan is now real and visible on the dashboard, but the Decision Engine itself doesn't yet factor 'is there already an in-flight plan for a competing mission' into its own verdict",
    ],
    northStarCritical: true,
  },
  reasoning: {
